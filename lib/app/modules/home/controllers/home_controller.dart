...
class HomeController extends GetxController {
  ...
  Future<List<Intervention>> getPendingTasks({bool showMessage = false
  , id
  }) async {
    List<Intervention> pendings=List();
    // pendings.value=[];
    ...
    // });
    bookings.clear();
    for(var element in intervention) {
      if (element.states == "pending") pendings.add(element);
    };
    if (showMessage) {
      Get.showSnackbar(
  ...
  Future<List<Intervention>> getOngoingTasks ({bool showMessage = false,id})async {
        currentIndex.value = id;
List<Intervention> goings=List();
    // goings.value=[];
    // bookings.clear();
    // for(var element in intervention) {
    //   if (element.states == "ongoing") goings.add(element);
    // };
    if (showMessage) {
      Get.showSnackbar(
          Ui.SuccessSnackBar(message: "Task page refreshed successfully".tr));
  ...
  Future<List<Intervention>>  getCompletedTasks({bool showMessage = false,id}) async {
     currentIndex.value = id;
List<Intervention> completeds=List();
    // completeds.value=[];
    bookings.clear();
    for(var element in intervention) {
      if (element.states == "completed") completeds.add(element);
    };
    if (showMessage) {
      Get.showSnackbar(
          Ui.SuccessSnackBar(message: "Task page refreshed successfully".tr));
